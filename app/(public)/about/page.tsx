  ...
  return (
    <>
      ...
      <Band max={1100}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(320px,100%),1fr))", gap: 44, alignItems: "center" }}>
          <div>
            ...
            <p style={{ margin: "20px 0 0", fontSize: 16.5, lineHeight: 1.7, color: "var(--argana-on-surface-muted)" }}>
            ...
            <p style={{ margin: "16px 0 0", fontSize: 16.5, lineHeight: 1.7, color: "var(--argana-on-surface-muted)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              {[
                ...
                <div key={x.l} style={{ padding: 18, borderRadius: 14, border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.022)" }}>
                  ...
                  <div style={{ fontSize: 12.5, color: "var(--argana-on-surface-muted)", marginTop: 4 }}>{x.l}</div>
                </div>
              ))}
            </div>
          </Card>
      ...
      <Band max={860} borderTop pad="clamp(48px,7vh,80px)">
        ...
        <p style={{ margin: "22px auto 0", maxWidth: 720, textAlign: "center", fontSize: 16.5, lineHeight: 1.7, color: "var(--argana-on-surface-muted)" }}>
          ...
          <Grid min={220} gap={14}>
            {FACTS.map((f) => (
              <Card key={f.label} hover={false} style={{ padding: 22 }}>
                <p style={{ margin: 0, fontSize: 11.5, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--argana-outline)" }}>{f.label}</p>
                <p style={{ margin: "8px 0 0", fontFamily: DISPLAY, fontWeight: 600, fontSize: 17, color: "var(--argana-on-surface)" }}>{f.value}</p>
              </Card>
            ))}
          </Grid>
          <p style={{ margin: "26px auto 0", maxWidth: 760, textAlign: "center", fontSize: 13.5, color: "var(--argana-on-surface-muted)" }}>
